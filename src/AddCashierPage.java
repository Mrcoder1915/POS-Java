...
public class AddCashierPage {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            JTextField searchBar = new JTextField("FILTER BY DATE OR YEAR") {
                protected void paintComponent(Graphics g) {
                    Graphics2D g2 = (Graphics2D) g.create();
                    g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                    g2.setColor(new Color(37, 57, 60)); // Dark green
            ...
            searchBar.setBounds(60, 40, 580, 30);
            searchBar.setForeground(Color.WHITE);
            searchBar.setFont(new Font("SansSerif", Font.BOLD, 13));
            searchBar.setOpaque(false);
            searchBar.setBorder(new EmptyBorder(5, 15, 5, 5));
            background.add(searchBar);

            JPanel itemList = new JPanel();
            itemList.setLayout(new GridLayout(18, 1, 0, 5));
            itemList.setBackground(Color.WHITE);
            itemList.setBounds(60, 80, 580, 390);

            for (int i = 0; i < 10; i++) {
                JPanel itemRow = new JPanel();
                itemRow.setLayout(new BorderLayout());
                itemRow.setBackground(new Color(240, 240, 240));
